...
pthread_mutex_t mutexAtaques; // Mutex para los ataques.
...
void iniciarTableros() {
	int i, j;
	if (pthread_mutex_init (&mutexAtaques, NULL) == 0) {
		for (i = 0; i < alto; i++) {
			for (j = 0; j < ancho; j++) {
...
void * entrarArena(void * concursante) {
	...
	while (personaje->hp > 0) {
		...
		//
		pthread_mutex_lock(&mutexAtaques);
		...
		pthread_mutex_unlock(&mutexAtaques);
	}
	pthread_mutex_lock(&tableroMovimientos[xPos][yPos]);
	tableroGuerreros[xPos][yPos] = -1;
	personaje->valido = 0;
	pthread_mutex_unlock(&tableroMovimientos[tmpX][tmpY]);
}
